...
\subsection{为雷达图绘制达标标准区域}  
\subsubsection{绘制达标标准区域命令}
\begin{NewMacroBox}{tkzThresholdLine}{\oarg{选项}}
用指定的参数绘制达标标准区域，该命令无必选参数。 
  
\medskip
\begin{tabular}{lll}
可选参数 & 默认值 & 含义               \\
\midrule
\TOline{threshold} {0.6}  {达标区域面积占总面积的比例}  
\TOline{fill}      {空}  {由数据点构成的多边形内部的填充色}
\TOline{opacity}   {0.5} {填充透明度}
\bottomrule
\end{tabular}
...
\begin{tkzexample}[]
\begin{tikzpicture}
   \tkzRadarDiagram[lattice=5]{A,B,C,D,E} 
